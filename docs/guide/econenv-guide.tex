...
\newcommand{\econenvversion}{1.3.0}
...
from a computer with no Python on it and finishes with all six programs
...
EconEnv turns the six programs into \emph{execution engines behind one Python
kernel}. You write:
...
\begin{lstlisting}
EconEnv 1.3.0 loaded - one notebook, multiple econometric engines.
  %econ                  econenv
  %Rec / %%Rec           econenv
...
The engine package must match the MATLAB release \emph{and} your Python.
Both windows are narrow:

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{MATLAB} & \textbf{Engine package} & \textbf{Python} \\
\midrule
R2024a & \texttt{pip install "matlabengine==24.1.*"} & 3.9--3.11 \\
R2024b & \texttt{pip install "matlabengine==24.2.*"} & 3.9--3.12 \\
R2025a & \texttt{pip install "matlabengine==25.1.*"} & 3.9--3.12 \\
R2025b & \texttt{pip install "matlabengine==25.2.*"} & 3.9--3.12 \\
R2026a & \texttt{pip install "matlabengine==26.1.*"} & 3.9--3.13 \\
\bottomrule
\end{tabular}
\end{center}

\begin{warn}
The Python range matters as much as the release. R2024a's engine supports
Python 3.9 to 3.11 and refuses anything newer, so a mismatch is an import
error rather than a wrong answer. Python 3.13 needs R2026a or later; on an
earlier MATLAB, make an environment your release can drive
(\texttt{conda create -n econ python=3.11}). \texttt{econenv doctor} works
this out for you --- it names a pin for every release you have installed, and
if none suits your Python it says which release would, and which Python your
own MATLAB can drive.
\end{warn}
...
GAUSS is driven through its terminal executable, \texttt{tgauss}. Everything in
this chapter was established by running GAUSS 26.1.1 rather than read from
documentation, which matters because several plausible assumptions turned out
to be wrong.

\section{Finding it}

Nothing to install beyond GAUSS itself. EconEnv looks at
\texttt{gauss.home}, then the \texttt{GAUSSHOME} and \texttt{MTENGHOME}
environment variables, then the usual locations.

\begin{warn}
GAUSS does \emph{not} install into Program Files on Windows. A typical path is
\texttt{C:\textbackslash gauss26}, so a search that only looks where other
vendors put things finds nothing. If yours is elsewhere:
...
Several GAUSS versions can be installed and they do not all work.
\texttt{\%econ doctor gauss} names the one EconEnv will actually use and lists
the others --- reporting the newest directory instead would repeat the mistake
the EViews adapter made with ProgIDs.

\section{Three things that catch everyone out}

\begin{enumerate}
  \item \textbf{Every statement ends with a semicolon.} A missing one is a
        syntax error, not a warning.
  \item \textbf{A bare expression prints nothing.} Where Python echoes the last
        value, GAUSS needs \texttt{print x;}.
  \item \textbf{\texttt{\textasciitilde} joins columns, \texttt{|} stacks
        rows.} This is how a design matrix is built and has no counterpart in
        the other engines' syntax.
\end{enumerate}

\begin{warn}
And one that looks like a bug: GAUSS \textbf{compiles the whole cell before
running any of it}. A mistyped name on the last line means the first line never
ran either, so a cell can produce no output at all rather than a partial
result. EconEnv says so explicitly, because the natural reading --- ``my print
statement did nothing'' --- sends people looking in the wrong place.
\end{warn}

\section{A least squares cell, annotated}
...
\texttt{/} is the least-squares solve, not division: GAUSS's equivalent of
\texttt{numpy.linalg.lstsq}, and more accurate than forming
\texttt{inv(X'X)X'y} by hand. For standard errors use \texttt{ols}, which is
what \texttt{compare\_ols} calls.

\section{Finding the command you need}
...
95 commands in 12 categories, 65 of them resolved against a live GAUSS. The
operators are catalogued too, because they are the part a Python or R user
cannot guess.

\section{Data, and precision}

A GAUSS matrix holds only numbers. A DataFrame's text columns cannot cross, and
they are \emph{named in a warning} rather than dropped in silence --- a
regression run on a frame that quietly lost a column is the worst outcome
available. Column names are kept on the Python side, so the frame comes back
with them. \texttt{NaN} crosses as a GAUSS missing value and returns as
\texttt{NaN}.

\begin{note}
Numbers cross at seventeen significant digits, which round-trips an IEEE double
exactly. GAUSS's own \texttt{csvWriteM} writes about fifteen, and a round trip
through it moves a value by roughly $3\times10^{-15}$ --- enough to put GAUSS
out of step with the machine-precision agreement that is the point of
\texttt{compare\_ols}. EconEnv ships its own writer for that reason, and the
measured round-trip difference is zero.
\end{note}

\section{What carries between cells}

Each cell is a fresh process, so EconEnv carries two things across: the
\emph{values}, using GAUSS's own \texttt{save} and \texttt{load}; and the
\emph{procedure definitions}, re-declared in each later cell. So a \texttt{proc}
written in one cell is callable in the next.

Carrying a definition is safe where replaying a statement is not: a
\texttt{proc ... endp;} block computes nothing and touches nothing, so
re-declaring it cannot change an answer. \texttt{\#include}s and
\texttt{library} statements still do not carry.

That limit is stated rather than worked around: replaying earlier cells to fake
a session would silently re-run their side effects.

\section{Figures}

Plots are captured and shown in the notebook. The default is SVG --- vector,
and what most journals ask for.
...
A cell that draws nothing produces no figure: EconEnv only asks GAUSS to save
a plot when the cell contains a plotting call, because \texttt{plotSave} would
otherwise write out whatever was drawn last and the same figure would reappear
under every later cell.

\begin{warn}
\texttt{plotSave} measures raster and vector differently: the two numbers are
pixels for PNG and inches for SVG and PDF. The \texttt{12 | 9} that gives a
sensible SVG gives a 12-by-9 \emph{pixel} PNG. EconEnv passes the right units
for the format you chose.

A \texttt{struct} also cannot be carried between cells --- GAUSS refuses to
\texttt{save} one. Since a plotting cell declares \texttt{struct plotControl p;},
EconEnv recognises struct declarations and leaves them out of the carried
workspace.
\end{warn}
...
GAUSS's own \texttt{ols} supplies the numbers. Maximum disagreement with Python
across six engines is $1.8\times10^{-15}$; $R^2$, adjusted $R^2$,
log-likelihood, AIC, BIC and RMSE match statsmodels, Stata and MATLAB exactly.
\texttt{ols} stops at coefficients, standard errors, sigma and $R^2$, so the
$t$ statistics, $p$-values, interval and information criteria are computed in
GAUSS from its own residuals using \texttt{cdftc} and \texttt{cdftci}.
...
What you pass can be a single model, a comparison across six engines, a
...
\section{All six engines, still in Colab}
